...
bool DiscordPresence::connectSocket()
{
    ...
    const QString tmp = qEnvironmentVariable("TMPDIR").isEmpty()
        ? QStringLiteral("/tmp") : qEnvironmentVariable("TMPDIR");
    for (const QString& name : names) {
        if (!runtime.isEmpty()) endpoints << QDir(runtime).filePath(name);
        if (!tmp.isEmpty()) endpoints << QDir(tmp).filePath(name);
        endpoints << name;
    }

    for (const QString& endpoint : std::as_const(endpoints)) {
        m_socket.abort();
