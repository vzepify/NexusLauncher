...
void Updater::checkIw4x()
{
    // IW4x has two independently published components:
    // the client DLL release and the rawfiles release.zip package.
    fetchJson(QUrl("https://api.github.com/repos/iw4x/iw4x-client/releases/latest"),
        [this](const QJsonObject& clientRelease) {
            const QString clientTag = clientRelease.value("tag_name").toString();
            if (clientTag.isEmpty()) {
                fail("No IW4x client release was found on GitHub.");
                return;
            }
            m_result.version1 = clientTag;

            fetchJson(QUrl("https://api.github.com/repos/iw4x/iw4x-rawfiles/releases/latest"),
                [this, clientTag](const QJsonObject& rawRelease) {
                    const QString rawTag = rawRelease.value("tag_name").toString();
                    if (rawTag.isEmpty()) {
                        fail("No IW4x rawfiles release was found on GitHub.");
                        return;
                    }
                    m_result.version2 = rawTag;

                    const bool dllCurrent = clientTag.compare(m_current1, Qt::CaseInsensitive) == 0;
                    const bool rawCurrent = rawTag.compare(m_current2, Qt::CaseInsensitive) == 0;

                    if (dllCurrent && rawCurrent) {
                        m_result = {true, false,
                                    "IW4x is already up to date (" + clientTag + ", rawfiles " + rawTag + ").",
                                    clientTag, rawTag};
                        emit progress(100, m_result.message);
                        emit finished(m_result);
                        return;
                    ...
                    if (!dllCurrent) {
                        emit progress(20, "A new IW4x client release is available: " + clientTag);
                        downloadIw4xDll(clientTag, !rawCurrent);
                    } else {
                        emit progress(20, "IW4x client is up to date. Checking rawfiles...");
                        if (!rawCurrent)
                            downloadIw4xRaw(rawTag);
                        else {
                            m_result = {true, false, "IW4x is already up to date.", clientTag, rawTag};
                            emit progress(100, m_result.message);
                            emit finished(m_result);
                        }
                    }
